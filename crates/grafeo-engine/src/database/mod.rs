...
impl GrafeoDB {
    ...
    fn build_sections(&self) -> Vec<Box<dyn grafeo_common::storage::Section>> {
        ...
        // Layered store: serialize compact base + overlay + catalog/indexes.
        // G-E2.RO: Catalog + VectorStore must be emitted after compact so RO
        // reopen restores shells + topology (early-return previously dropped them).
        #[cfg(all(feature = "compact-store", feature = "lpg"))]
        if let Some(ref layered) = self.layered_store {
            ...
            let overlay = layered.overlay_store();
            let overlay_section = grafeo_core::graph::lpg::LpgStoreSection::new(Arc::clone(&overlay));
